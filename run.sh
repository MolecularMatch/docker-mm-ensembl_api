...
aws s3 cp s3://mm-s3-bucket/importdata/ensembl_api_86_cache_fasta.tgz ensembl_api_86_cache_fasta.tgz

#unzip the files from s3 to the proper location
if [ -e "ensembl_api_86_cache_fasta.tgz" ]
then
	mkdir -p /opt/.vep
	tar -xvf ensembl_api_86_cache_fasta.tgz -C /opt/.vep
fi
