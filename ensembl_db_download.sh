...
function clone_ensembl_db {
  ...
  if [ ! -e ${sourcedir}/${dbname}.downloaded ]; then
    rsync -avP rsync://ftp.ensembl.org/ensembl/pub/${ftpsubpath}/${dbname} ${sourcedir}/ &&\
  ...
  if [ ! -e ${sourcedir}/${dbname}.dbcreated ]; then
      mysql -u mmadmin -p1MoreMatch! -h ${dbhost} -P ${dbport}\
  ...
  if [ ! -e ${sourcedir}/${dbname}.schemaloaded ]; then
      ...
      mysql -h ${dbhost} -P ${dbport} -u mmadmin -p1MoreMatch! ${dbname} < ${dbname}.sql &&\
      gzip ${dbname}.sql &&\
      cd - &&\
  ...
  if [ ! -e ${sourcedir}/${dbname}.dbloaded ]; then
      ...
      if [ $? -eq 0 ]; then
        cd ${sourcedir}/${dbname} &&\
        gunzip *.txt.gz &&\
        mysqlimport -h ${dbhost} -P ${dbport} -u mmadmin -p1MoreMatch! --fields_escaped_by=\\ ${dbname} -L *.txt &&\
        gzip *.txt &&\
        cd - &&\
...
# skip ensembl_accounts if it already exists in the instance. We are installing v84 in parallel with v83 so we have ensembl_accounts already
#for database in ensembl_ontology_${release} ensembl_website_${release} ensembl_accounts ensembl_compara_${release}; do
# skip compara for now. It's very large and very optional
#for database in ensembl_ontology_${release} ensembl_website_${release} ensembl_compara_${release}; do
for database in ensembl_ontology_${release} ensembl_website_${release}; do
  clone_ensembl_db ${database} ensembl.cdg5r2nfuxax.us-east-1.rds.amazonaws.com 3306 "current_mysql"
done
...
# By Specie
 for database in ${specie}_core_${release}_${subrelease} ${specie}_funcgen_${release}_${subrelease} ${specie}_otherfeatures_${release}_${subrelease} ${specie}_variation_${release}_${subrelease}; do
  clone_ensembl_db ${database} ensembl.cdg5r2nfuxax.us-east-1.rds.amazonaws.com 3306 "grch37/release-${release}/mysql"
 done